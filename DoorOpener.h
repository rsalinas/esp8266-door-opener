...
public:
    DoorOpener(int relay, int led) : relay_(relay), led_(led) {
        ...
        digitalWrite(OPEN_SIGNAL_LED, HIGH);  // Turn the LED off by making the voltage HIGH
    }
    void activate() {
        ...
        relayTicker.once<DoorOpener*>(1.0, [](DoorOpener* self) {
            if (self->led_ >= 0)
                digitalWrite(self->led_, HIGH);  // Turn the LED off by making the voltage HIGH
